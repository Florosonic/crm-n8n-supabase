...
BEGIN
    ...
    -- Поиск существующей сделки: сначала пробуем найти с новым форматом (название_компания),
    -- затем пробуем найти по старому формату (только название)
    SELECT id, stage INTO v_deal_id, v_old_stage
    FROM deals
    WHERE company_id = p_company_id
      AND status = 'open'
      AND (
        -- Новый формат: полное совпадение с подчёркиванием
        LOWER(TRIM(REGEXP_REPLACE(product_name, '\s+', ' ', 'g'))) = LOWER(v_product_name)
        OR
        -- Старый формат: совпадение только названия продукта (до подчёркивания)
        LOWER(TRIM(REGEXP_REPLACE(product_name, '\s+', ' ', 'g'))) = LOWER(REGEXP_REPLACE(v_product_name, '_.*$', '', 'g'))
      )
    LIMIT 1;
